---
author: sumanic
description: Learn about how to set up the Microsoft Dynamics 365 Finance + Operations (on-premises) provider in Dynamics 365 Intelligent Order Management.
...
This setup enables order synchronization between Intelligent Order Management and Finance + Operations, in both directions.

- Set up dual-write in your Finance + Operations instance. For information about how to set up dual-write, see [Guidance for dual-write setup](/dynamics365/fin-ops-core/dev-itpro/data-entities/dual-write/connection-setup).
- Install Intelligent Order Management in the same Dataverse instance that hosts Finance + Operations.
- Enable mappings that are associated with dual-write in Intelligent Order Management.
    ...
    1. Set the dual-write mappings to enable synchronization from Intelligent Order Management to Finance + Operations. You must add order mapping filters to delay order synchronization to Finance + Operations when an order isn't ready to sync.
...
- Order synchronization from Intelligent Order Management to Finance + Operations requires that some key parameters be sent in the order:
  ...
  - **Sales Order Product** policy parameters:
    ...
    You can pass these parameter values through policy rule definitions in Intelligent Order Management, as shown in the following example for a sales order policy.
...
1. In the **Connections** section, set up one connection: **Microsoft finance and operations Dataverse (current environment) Connection**.
...
1. Select **Save and close**.
1. Go to **Providers \> Installed**, and verify that the provider you set up is listed and has a status of **Activated**.

> [!NOTE]
> Only orders that are confirmed in Finance + Operations can sync to Intelligent Order Management. To help ensure that an order from Finance + Operations syncs successfully to Intelligent Order Management, first verify that the order is confirmed in Finance + Operations.
