...
|---------|-------------|------|
| 1.1.3382.2 | <ul><li>Add Order Creation Queue to the IOM sitemap with updated list and detail views to improve visibility into queue operations.<li>Introduce actionable error handling for inbound order queue failures. Orders that reach max retries now surface in Provider Inbound Errors as a new InboundOrderQueueError category and can be reprocessed.<li>Add Message ID to the order queueing pipeline, stored on the queue record and preserved through retries for end-to-end order traceability.<li>Add Application Insights telemetry for unified step execution, logging begin and finished events with entity record ID and failure reason.<li>Add Application Insights telemetry for the Send to Fulfilment pipeline, capturing order ID, order number, line counts, and per-line validation failure reasons.<li>Fix IOM installation failure in Australia preprod (AUP) environments by adding AUP region support to the package deployer.<li>Fix incorrect privilege name casing in security roles for the web session challenge entity, resolving Power Automate flow permission errors.<li>Fix missing state column on fresh installation.</ul> | April 2026 |
| 1.1.03322.1  | <ul><li>Introduce automated order queue retry handler in the throttling wrapper that tracks queued orders by execution state, expiration timestamps, and failure counts to automatically retry orders that failed or expired during async processing, or log them as persistent errors after reaching retry limits.<li>Implement smart retry logic within unified flow execution for batch update operations (xMultiple) that automatically retries failed batch requests up to two times with intelligent delay extraction before falling back to individual request processing, handling Microsoft Dataverse throttling, Microsoft Power Automate timeouts, finance and operations apps throttling, and dual-write synchronization errors.<li>Add timeline validation to prevent business event crashes on entities without timeline enabled, and fix provider activation failures by detecting duplicate ProviderAction records before insertion to prevent SQL constraint violations.<li>Update security dependencies to address known vulnerabilities and maintain system security compliance.</ul> | Feb 2026 |
...
| 1.1.1.1033 | <ul><li> Deliver localization updates to improve global readiness and consistency.<li> Apply security hardening improvements, accessibility compliance ,including package signing and dependency updates. | Oct 2025 |
