---
author: josaw1
description: Learn about inventory-related dashboards in Microsoft Dynamics 365 Intelligent Order Management.
ms.date: 01/28/2026
ms.custom: 
  - bap-template
ms.topic: how-to
...
To view inventory dashboards for your Intelligent Order Management organization, first ensure that your organization's Intelligent Order Management build version is 1.0.0.4354 or later.

Dashboards provide useful information only if your Intelligent Order Management organization is configured to use the Inventory Visibility service. For more information, see [Inventory Visibility Add-in overview](/dynamics365/supply-chain/inventory/inventory-visibility).
...
1. On the **Feature management** tab on the right side, turn on the **OnHandMostSpecificBackgroundService** option. This setting ensures that data flows from the Inventory Visibility service into Intelligent Order Management, so that the dashboard can consume it. Note that turning this flag off after the dashboard is functional causes the dashboard to display incorrect information. The data is picked up from the time the flag is turned on and doesn't consider the prior data before the flag was turned off.
...
This section describes the visualizations that inventory dashboards provide: filters, KPIs, charts, and tables.

### Filters

The dashboard provides two interactive filters: **Product name** and **Location ID**. Use these filters to focus on one or more products or locations. They refine your data so that the KPIs and charts reflect the selected products or locations.
...
- **Physical available** – The value is the sum of all units that are physically available in the warehouse. It includes items that are "soft reserved." The value of this KPI updates whenever inventory data is uploaded into the system. When items ship from the warehouse, the number of those items is deducted from the KPI value.
- **Soft reserved** – After fulfillment optimization occurs and the warehouses for sales orders are determined, the system soft reserves the appropriate items in that warehouse. When you soft reserve items, add the number of those items to the value of this KPI, to ensure that the same items aren't double booked against two different orders. After soft-reserved items ship, deduct the number of those items from the KPI value.
...
- **Returned** – The value is the number of product units that are returned to the warehouse after they were sold to customers.
- **Return rate** – The value is the return rate of product units that are returned to the warehouse after they were sold to customers. It's calculated as (*Units returned* &divide; *Units sold*) &times; 100.
...
Currently, the inventory dashboard is more suitable as a daily report that you can use to assess the health of a business's inventory than as a real-time representation of inventory information. You can expect 4 to 10 hours of latency for the information that flows into the inventory dashboard. A feature that provides real-time information on the inventory dashboard is planned for an upcoming release.

### Can I view data for specific products that I care about?

Yes. Use the **Product name** and **Location ID** filters to refine the product data that is represented in the KPIs and charts.

### Can I sort the data differently than what is available out of the box?

Yes. When you hover over a chart, a vertical ellipsis appears in the upper-right corner. To resort the data, select the ellipsis, select **Sort axis**, and then select the option that you want.

### Can I modify the KPIs that the dashboard provides?

Currently, only the KPIs described in the [KPIs](#kpis) section are available. Market research showed that these KPIs are the most useful for most businesses. If you have suggestions for new KPIs or charts, or changes to the existing KPIs and charts, contact the team at <D365OMS@microsoft.com>.

### I'm unsure how some of these KPIs are calculated. How do I get more information?

The [KPIs](#kpis) section describes how these KPIs are calculated. You can also hover on an inventory dashboard KPI number (not the KPI name) to open a tooltip that describes how that KPI is calculated.
...
No. Currently the dashboard only supports the IV datasource. This limitation means you can't use the dashboard when your inventory is managed within dynamics supply chain management.
